    ...
    const menu = data
      .map((item) => {
        if (item.submenu) {
          ...
          <a href="${ChangeToSlug(item.name)}" class="nav-link dropdown-toggle" data-toggle="dropdown">
          ${item.name}<i class="fa fa-angle-right float-right mt-1"></i>
          </a>
          ...
          return `<a href="${ChangeToSlug(item.name)}" class="nav-item nav-link">${item.name}</a>`;
        }
      })
      .join("");
